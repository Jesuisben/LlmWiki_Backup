...
```
-> 5\*3이라는 컬럼(Column)에 1이라는 행(row)에 값이 15로 나옴
...
```
-> upper는 대문자로 바꾸기 / lower는 소문자로 바꾸기
...
```sql
SELECT count(*) - COUNT(PRICE) AS "단가널갯수" FROM PRODUCTS;
```
-> 쌍따옴표는 파생컬럼의 이름을 정할때 사용함
