...
# sort() : 리스트 내부의 순서를 정렬하는 함수
# 기본값은 오른차순 / sort(reverse=True) 하면 내림차순 정렬됨
words.sort()
print(words)
...
# 일반 문자열 결과: ['\hello', '\world', '\python'] 이렇게 나와야할거같은데 역슬래시가 2개 붙은 이유  
# 파이썬에서 리스트를 출력하면 각 문자열을 repr() 형태(개발자용 표현)로 보여줌  
# repr()는 문자열을 개발자가 정확하게 확인할 수 있는 형태로 출력함  
# 그래서 특수문자(줄바꿈, 탭, 역슬래시 등)를 이스케이프 표기법으로 표현함  
#  
# 예를 들어 실제 문자열이 "hello\nworld"라면  
#  
# print() 결과  
...
# 'hello\nworld'  
#  
# 같은 원리로 실제 문자열의 역슬래시(\)도 repr()에서는 \\로 표시됨  
# 따라서 화면에는 \\hello처럼 보이지만 실제 문자열은 \hello 임  
  
# 검증을 하기 위해 해당 리스트의 요소들을 하나하나 꺼내서 print해봄  
print("--- 진짜 알맹이 확인하기 ---")  
# 리스트 껍데기를 벗기고 단어를 하나씩 순수하게 출력해 보면? 다음과 같이 됨  
for word in result1:  
...
# 1. 코드 작성      : r'\\([^\\]+)$'# 2. 정규식 엔진 수신 : \\([^\\]+)$  (raw이므로 변환 없음)  
