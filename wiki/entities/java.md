...
- **Java 수업에서 직접 확인한 것:** 값을 타입으로 구분하고, 조건·반복으로 로직을 만들며, 배열로 묶고, 클래스·상속·인터페이스로 객체 구조를 설계했다.
- **프로젝트에서의 역할:** 후속 Spring 프로젝트의 Controller·Service·DTO·Entity도 Java 클래스이므로, 생성자·접근 지정자·컬렉션·다형성 이해가 구현과 오류 해석의 기반이 된다.
- **면접 설명 기준:** “객체지향 언어”라고만 답하기보다 `Product`와 `Beverage` 수업 예시로 캡슐화, 상속, 다형성, 추상화를 각각 어디서 사용했는지 설명한다.
