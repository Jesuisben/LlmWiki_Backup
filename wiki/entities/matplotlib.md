...
2026-06-30~07-01 수업에서는 Pandas `plot()`으로 기초 그래프를 맛봤고, 2026-07-03에는 복지 데이터와 소득 데이터를 이용해 본격적으로 여러 그래프 유형을 사용했다. 특히 `plt.rc('font', family='Malgun Gothic')`로 한글 폰트를 지정하고, `plt.title()`, `plt.xlabel()`, `plt.legend()` 같은 설정을 함께 다뤘다. ^[raw/KoreaICT/10. Python/2026.07.03(금)/2026.07.03(금).md]
