...
- local image를 registry 형식으로 tag하고 login·push한 뒤 다른 환경에서 namespace를 포함해 pull/run하는 수동 전달 흐름을 실습했다.
...
- MySQL과 Spring container를 같은 network에 연결해 container 상태, Spring log, DB row, browser 9000 응답을 별도로 확인했다.
...
- `up`의 자원 생성, container 상태, DB 작업, browser 응답, `down` 뒤 image·volume 잔존을 단계별로 확인했다.
...
| Compose project | image와 service 관계 YAML | project network·volume·container 생성/정리 | `Up`·DB·browser·`down`·잔존 자원 구분 |
...
- Compose로 두 종류의 다중 service 실행·DB/browser 확인·정리
