...
Java와 Spring Boot에서는 코드를 작성하고 실행하는 개발자 관점이 중심이었다. Linux 과정부터는 서버에 접속하고, 파일을 배치하고, 권한을 관리하고, 포트를 열고, 배포 산출물을 실행하는 운영 관점이 추가된다.

## 학습 이력

- [[summaries/2026-04-22-linux-install-ssh-cli|2026-04-22]]: VirtualBox Ubuntu 설치, SSH 서버, MobaXterm 접속, 프롬프트와 경로.
- [[summaries/2026-04-23-linux-files-vi|2026-04-23]]: 파일/디렉터리 명령어, `vi`, 복사·이동·검색.
- [[summaries/2026-04-24-linux-users-permissions|2026-04-24]]: 사용자, 그룹, 권한, `/etc/passwd`, `/etc/shadow`, `/etc/group`.
- [[summaries/2026-04-27-linux-archive-java-alias|2026-04-27]]: 다운로드, 압축, 삭제, alias, Java 실행 준비.
...
- 사용자/권한 관리: owner/group/others, root, sudo, useradd.
- 서버 프로세스 관리: `systemctl`, 방화벽, 포트.
- 배포 준비: Maven, Java, Docker, Git과 연결된다.

## 프로젝트/면접 관점

Linux를 “명령어 몇 개”로만 외우기보다, 웹 서비스를 서버에 올리기 위한 운영 환경으로 설명하면 좋다. 예를 들어 “Spring Boot jar를 Linux 서버에서 실행하고, 포트/방화벽/권한을 확인할 수 있다”가 실무적인 설명이다.
