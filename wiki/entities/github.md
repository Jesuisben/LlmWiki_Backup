...
- **CI/CD 후속:** 05-11부터 GitHub Actions workflow와 repository-level Secrets가 사용된다. 이는 Linux PR 실습과 별도의 자동화 학습이며, 원본에는 workflow·Actions 화면 확인 절차가 있지만 job 성공 log는 없다.
- **단계 9 중간 프로젝트:** backend/frontend workflow template와 registry·server·DB·JWT·object storage Secret 역할을 한 프로젝트 배포 가이드로 조립했다. 문서 안 template과 항목 목록은 있으나 실제 repository 설정·Actions run·artifact는 없다.
...
- [[summaries/2026-05-11-cicd-github-actions-spring-boot|2026-05-11]] — workflow 정의와 Docker/EC2 Secrets, container 결과를 구분한 후속 자동화
