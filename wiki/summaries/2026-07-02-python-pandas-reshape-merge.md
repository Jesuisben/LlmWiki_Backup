...
수업에서는 `result = pd.merge(data01, data02, on=on_condition, how='outer', suffixes=('_중간', '_기말'))`로 중간·기말 접미사를 직접 지정했다.
...
수업에서는 `_merge == "left_only"` 조건을 `query()`에 전달해 시험 점수 쪽에 대응 행이 없는 학생을 찾았다.
...
예제는 `name`을 index, `item`을 columns, `value`를 values로 지정했고, 다음 단계에서는 `value`와 `qty` 두 값을 함께 펼쳤다.

`pivot`은 보고서나 그래프를 만들기 전에 데이터를 보기 좋은 표 형태로 바꾸는 데 자주 쓰인다.

### 7. melt와 pivot_table까지 확장

직원별 월 실적에서는 wide format을 `melt`로 long format으로 바꾼 뒤 특정 사람·월을 필터링했다. 영화 매출에서는 `pivot_table`로 구·연도·월·장르를 평균·합계로 집계하고, 다중 columns를 단일 이름으로 정리했다. 따라서 이날의 책임은 단순 `pivot` 소개가 아니라 long/wide 변환과 집계형 재구조화까지 포함한다.

### 8. DataFrame 연산·조회로 연결

후반에는 `payment02.csv`로 여러 열 선택, `select_dtypes`, `filter`, columns 재배치, `describe`, 결측 개수, axis별 합계, `nlargest`·`nsmallest`, `query`를 연습했다. 이는 결합한 표가 끝이 아니라 분석 가능한 columns와 행을 선별하는 단계로 이어진다는 뜻이다.

### 실행 증거 경계

노트에는 직접 타이핑한 code와 일부 DataFrame output 문맥이 있지만 독립 `.ipynb` 파일과 저장 graph artifact는 raw에 없다. `read_csv`, `plot` 호출만으로 전체 notebook 재실행·그래프 저장 성공을 단정하지 않는다.

## 핵심 개념

- [[concepts/pandas-dataframe-basics|Pandas DataFrame 기본]]: 07-01까지의 단일 DataFrame 조회·수정·입출력 기반이다.
- [[concepts/pandas-dataframe-reshape-merge|Pandas DataFrame 결합과 재구조화]]: `concat`·`merge`·`pivot`·`melt`·`pivot_table`의 책임을 독립적으로 정리한다.
