...
CI/CD 배포의 앞단 인프라가 되는 도메인, Route 53 hosted zone, ACM 인증서, ALB/Target Group, HTTPS 접속 흐름을 복습하고 실습한 날이다.
...
AWS NS record 4개 확인
  ↓
가비아 네임서버에 AWS NS 값 등록
  ↓
ACM 인증서 요청
  ↓
DNS 검증 CNAME record 생성
  ↓
인증서 발급 확인
```
...
HTTP보다 HTTPS를 사용하는 이유는 인증 정보와 요청 데이터 보호 때문이다. ACM에서 루트 도메인과 wildcard 도메인(`*.도메인`)을 포함해 인증서를 요청하고, DNS 검증용 CNAME record를 Route 53에 추가한 뒤 인증서 상태가 `발급됨`인지 확인했다.

### 3. Load Balancer와 Target Group

Load Balancer는 여러 EC2로 요청을 나누고, 한 서버에 문제가 생겨도 다른 서버가 응답할 수 있게 하는 부하 분산 장치다. 수업에서는 2A/2C EC2를 준비하고, Target Group에 인스턴스를 묶은 뒤 ALB를 생성했다.
...
Route 53 hosted zone에서 루트 도메인과 `www` subdomain 각각에 A/Alias record를 만들고 ALB로 연결했다. 최종 확인은 `http://도메인`, `http://www.도메인`, `https://도메인`, `https://www.도메인` 네 가지 주소로 수행했다.
