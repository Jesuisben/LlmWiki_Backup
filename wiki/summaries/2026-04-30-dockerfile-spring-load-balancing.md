...
7. container 상태, Spring log, browser 화면을 차례로 확인한다.
...
- Spring log에서 application 시작 메시지와 DB 원인 예외가 없는지 확인했다.
- browser에서 host의 9000 포트로 product 화면을 확인했다.
- 실습 뒤 두 container를 중지·삭제하고 목록이 비었는지 확인했다.

image build 성공, container `Up`, application log 정상, browser 응답은 서로 다른 확인 단계다. 하나만 통과했다고 전체 연결 성공으로 보면 안 된다.
