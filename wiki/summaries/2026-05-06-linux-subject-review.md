...
- 결과: container `Up`과 DB row 조회는 보존됨. Spring start log와 browser 9000은 확인 기준·명령·URL만 있고 결과는 미보존

### 여러 service를 Compose로 복원

- 입력: 준비된 image와 service 관계를 적은 YAML
- 처리: Compose가 network·volume·container를 생성하고 환경 변수를 전달
- 결과: 첫 실습의 상태 목록과 DB 작업은 보존됨. browser와 `down` 이후 상태, WordPress 실습 결과는 미보존
...
- AWS: VPC·Subnet·Security Group·EC2·RDS와 cloud 비용·자원 생명주기
- CI/CD 후속: Route 53·ACM·ALB·Target Group과 GitHub Actions 자동 배포
