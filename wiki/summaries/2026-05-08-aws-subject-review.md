...
날짜 원본은 05-07에 EIP·EC2·Security Group·VPC·Key Pair 해제 절차와 반복 실습 지시를, 05-08에 RDS를 포함한 정리 순서를 남겼다. 다만 반복별 삭제 완료 화면·최종 자원 목록은 없으므로 “절차 학습”과 “최종 상태 확인”을 같은 결과로 보지 않는다.

### 5. 이후 CI/CD·도메인 배포로 이어지는 이유

AWS 수업 단계에서는 EC2 접속, 빌드, 서버 실행, Nginx 설정을 사람이 직접 수행했다. Load Balancer·Route 53·ACM 구성은 05-12 CI/CD 후속 수업이며, GitHub Actions는 push 뒤 build와 배포 절차를 반복 가능하게 실행하는 방향으로 발전한다. 그래서 AWS 총정리는 CI/CD 이전의 “수동 EC2·RDS 배포 구조”를 복습하는 위치에 있다.
...
- 05-08에서 출력으로 성공이 확인되는 것은 ICMP 규칙 추가 뒤 양방향 `ping`이다. Nginx 브라우저, Spring Boot jar 응답, RDS SQL/JDBC 애플리케이션 결과는 명령·절차와 빈 확인 칸만 남아 있다.
