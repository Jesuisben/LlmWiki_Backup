...
`payment07.csv`를 읽고 `payment.groupby("성별")["교통비"]`로 그룹 객체를 만든 뒤 합계와 건수를 각각 표로 확인했다.
...
교통비에는 합계·평균·최댓값, 출장기간에는 평균·최솟값을 적용하는 집계 사전을 만들고, 출장지역·성별 기준 `groupby().agg()`에 전달했다.
...
수업에서는 `get_level_values()`로 바깥·안쪽 column level을 분리하고 집계 함수명을 한글로 매핑한 뒤 언더스코어로 결합했다. 이어 `reset_index()`로 행 MultiIndex를 일반 columns로 되돌렸다.
...
교통비가 하한과 상한 사이인지 boolean으로 만든 뒤 평균을 구하는 사용자 정의 함수를 작성하고, 성별 그룹에 추가 인수와 함께 전달했다.
...
이름·월별 점수에서 첫 주 값을 기준으로 향상률을 계산하는 함수를 `transform()`에 적용하고, 원본과 같은 길이의 결과를 `향상_비율` 파생 column으로 붙였다.
...
소득 구간 경계와 저소득~고소득 label을 준비해 `pd.cut()`으로 `범주형소득` 파생 column을 만들고, 성별·소득구간별 평균을 집계했다.
...
노트에는 code와 일부 `info()`·DataFrame 표현이 함께 있지만, 이날 날짜 폴더에는 graph PNG나 독립 `.ipynb`가 없다. 따라서 집계·plot code를 작성하고 노트에서 결과를 관찰한 수업 흐름은 확인되지만, 모든 chart가 저장되었거나 전체 notebook이 순서대로 재실행되었다고 단정하지 않는다.
