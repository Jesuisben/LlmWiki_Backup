...
- 로컬 Windows 개발 환경에는 MariaDB, JDK, STS/Eclipse, Maven 프로젝트를 준비했고, `userinfo` 테이블에 샘플 사용자 정보를 저장하도록 구성했다.
...
- 뒤쪽 실습에서는 Maven으로 만든 WAR 파일을 Rocky Linux Tomcat `webapps`에 배치하고, 설정 수정 뒤 서버 재시작과 브라우저 동작 확인까지 수행했다.
...
수업에서 특히 중요한 관찰은 Passwordless 등록 후 일반 비밀번호 로그인 동작이 달라진다는 점이다. **이 샘플에서는** Passwordless 로그인 사용 시 비밀번호가 임의 값으로 바뀌어, 이후 일반 비밀번호 로그인을 하려면 Passwordless 등록 해제가 필요했다. 이는 X1280 일반 규칙이 아니라 수업 샘플에서 확인한 동작이다.
