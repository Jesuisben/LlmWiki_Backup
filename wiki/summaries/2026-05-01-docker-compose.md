...
전날 수동으로 만들었던 MySQL·Spring Boot의 image·network·volume·environment·port 관계를 Compose YAML에 선언하고 `up`으로 생성했다. 자원 생성·container `Up`·DB 결과는 보존됐고, browser·`down` 이후 상태와 WordPress 실습은 확인 절차만 남아 있다.
...
- `up -d`, browser 8085 확인 URL, `down`, image·volume 삭제 명령을 순서대로 기록했다. 실행 출력과 WordPress 화면 결과는 보존되지 않았고, volume 삭제 명령의 이름도 manifest 선언과 달라 실제 삭제 성공으로 확정하지 않는다.
...
- Spring browser 확인 URL 기록; 실제 화면 관찰 결과는 미보존
- `down`과 image·volume 확인·삭제 명령 기록; 실행 결과는 미보존

이 흐름에서 자원 생성·두 container의 `Up`·DB row 입력과 조회는 결과가 보존됐다. browser와 `down` 이후 상태는 확인 절차만 기록됐으므로 같은 완료 수준으로 묶지 않는다.
...
날짜 MD에 두 manifest와 실행 절차가 충분히 전사되어 있어 보조 교안을 page source로 강제하지 않았다. Spring Compose의 자원 생성·`Up`·DB 결과와, 결과가 미보존된 browser·`down`·WordPress 절차를 구분했으며 실제 credential·계정·IP는 재노출하지 않았다.
