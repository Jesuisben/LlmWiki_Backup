...
- 테스트 모드는 DNS TXT 검증 없이 승인 상태가 되므로 수업 실습에서 사용했다.
...
수업에서는 Docker Hub의 X1280 통합 이미지를 실행하고 여러 포트를 열어 서버 3종을 확인했다. Docker `-p` 포트 매핑과 Docker Host의 `firewalld` 허용은 별도 설정이며, 외부 접근 포트는 Host 방화벽에서도 열어야 했다. 위키에는 원본의 실습 IP·서버키·관리자 비밀번호 같은 값은 보존하지 않고, 역할만 남긴다.
