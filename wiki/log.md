...
## [2026-07-22] create | Java 인간용 통합 총정리와 Study Guides 레이어

- 사용자가 과목마다 한 파일만 열어 처음부터 끝까지 복습하기를 원한다는 목적에 맞춰 `wiki/study-guides/`를 인간용 통합 교재 레이어로 신설했다.
- `java-complete-review`를 생성해 2026-02-26~03-13 Java의 실행 구조·변수/타입·연산자·입력·조건/반복·배열·클래스/객체·생성자/캡슐화·상속/다형성·추상 클래스/인터페이스·메모리 관점·`static`/`final`·주변 Git/Oracle 연결을 한 파일에 통합했다.
- 간단한 코드는 실제 수업 결과를 가장하지 않는 `복습용 재구성 예제`로 표시하고, 혼동 비교·직접 작성 과제·자기 점검 체크리스트를 함께 넣었다. 기존 Java Summary·Concept·Comparison과 raw 원본은 수정하지 않았다.
- `AGENTS.md`에 `study-guides/`와 `type: study-guide`의 목적·작성·예제·장문 예외 규칙을 추가하고, 사용자 학습 가이드의 과목 전체 복습 시작점을 Study Guide로 바꿨다. Index `Study Guides` 섹션에 등록하고 `Total pages`를 313으로 갱신했다.
