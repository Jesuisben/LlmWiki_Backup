...
이 Wiki는 모든 파일을 처음부터 끝까지 읽는 교과서가 아니다. **과목 전체 흐름을 잡고, 필요한 개념을 따라가며, 부족할 때 원본을 확인하는 개인 학습 지도**다.

가장 기본적인 학습 순서는 다음과 같다.

> 과목 총정리 → 관련 개념·비교 페이지 → 필요한 날짜별 수업 요약 → 실제 코드가 필요할 때 raw 원본

파일이 많아도 한 번에 읽을 것은 보통 **총정리 1개와 관련 페이지 2~3개**면 충분하다.

## 지금 무엇을 열어야 할까

| 공부 목적 | 가장 먼저 볼 페이지 | 다음에 볼 페이지 |
|---|---|---|
| 과목 전체 복습 | 과목별 총정리 Summary | 핵심 Concept와 Comparison |
...
과목을 다시 공부할 때는 다음 총정리 페이지 중 하나부터 연다.
...
총정리에서 과목의 흐름을 먼저 파악한 뒤, 이해가 약한 위키링크만 따라간다. 총정리 안의 모든 링크를 한 번에 열 필요는 없다.
...
1. 과목 총정리를 읽는다.
2. 약한 주제 세 개를 고른다.
3. 각 주제의 Concept를 읽는다.
4. 관련 Comparison이 있으면 차이를 말로 설명한다.
5. 필요한 날짜 Summary만 돌아가서 수업 맥락을 확인한다.
...
> **과목 전체는 총정리, 특정 날짜는 Summary, 이해는 Concept, 차이는 Comparison, 실제 원문은 Raw에서 확인한다.**
