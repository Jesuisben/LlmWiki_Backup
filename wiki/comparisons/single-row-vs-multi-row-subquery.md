...
1. 전체 평균 급여는 값 하나이므로 “평균보다 많이 받는 회원”은 `salary > (SELECT AVG(...))`처럼 단일행 비교를 쓴다.
2. 김구·유관순 두 관리자의 ID는 여러 값이 될 수 있으므로 “둘 중 한 명이 관리하는 회원”은 `manager IN (SELECT id ...)`처럼 다중행 비교를 쓴다.

## 흔한 오해

- 서브쿼리를 문법 모양만 보고 단일행이라고 판단하지 않는다. 실제 조건상 몇 행이 나올 수 있는지를 먼저 본다.
- 다중행 결과에 `=`을 쓰는 오류는 괄호를 추가해서 해결되지 않는다. 연산자를 `IN`/`ANY`/`ALL` 중 질문에 맞게 바꿔야 한다.
- `IN`은 언제나 다중행 전용이라는 뜻이 아니라 여러 후보 중 포함 여부를 표현한다. 결과가 한 개인 경우에도 동작할 수 있지만 의도를 더 직접 드러내는 연산자를 선택한다.
