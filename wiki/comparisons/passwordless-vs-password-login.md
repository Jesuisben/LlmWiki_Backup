...
| [[summaries/2026-05-18-passwordless-x1280-server-spring-sample|2026-05-18]] | 외부 인증 서버·MariaDB·Spring 샘플에서 등록/로그인/해제를 관찰 | X1280 Docker 서버와 샘플 앱 |
| [[summaries/2026-05-21-passwordless-x1280-rest-api|2026-05-21]] | Postman으로 등록 여부 등 API 응답 계약을 확인 | X1280 REST API 단위 검증 |
...
| 직접 구현 범위 | 04-06 준비, 04-07 login/filter/SecurityContext 직접 연결 | 05-14 개념, 05-18 샘플 관찰, 05-21 Postman API 확인; 전체 React/Spring 통합은 후속 가이드 |
...
중간 프로젝트 가이드는 외부 인증 서버 설정, REST client, DTO, Service, Controller, SecurityConfig, React 상태 연결을 제안한다. 이때 X1280 승인 성공을 기존 서비스의 로그인 상태·JWT/세션과 어떻게 연결할지 별도로 설계해야 한다. 이 전체 설계를 04-06~07 또는 05-18 날짜 수업의 직접 구현으로 소급하지 않는다.
...
| 프로젝트 통합 | 승인 상태·외부 API 응답 | REST client·DTO·Service·Controller·SecurityConfig와 frontend 상태 연결 | 기존 서비스 로그인 상태와 연결하는 후속 설계 |
...
- Passwordless 흐름: 05-14 개념, 05-18 X1280 Docker 서버와 Spring 샘플의 등록·로그인·해제 관찰, 05-21 Postman API 응답 확인.
...
- 05-21 Postman API 성공만으로 프로젝트 login이 완성된 것은 아니다.
...
- **Passwordless 직접 수업:** 05-14 개념, 05-18 서버·샘플, 05-21 Postman API 확인이다. 이 페이지는 비교에 필요한 수준만 사용하며 8과목 전체를 재고도화하지 않는다.
