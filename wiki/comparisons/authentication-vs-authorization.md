...
FrontEnd_BackEnd 수업에서는 04-06의 로그인 입력·JWT 구성 준비, 04-07의 credential 인증·후속 Bearer 검증, 04-14의 인증 사용자 Cart 처리, 04-17~20의 role별 주문 화면이 차례로 등장했다. 이 흐름에서 **사용자를 확인해 현재 요청의 `Authentication`을 만드는 일**과 **그 사용자가 특정 endpoint·자원·기능을 실행해도 되는지 판단하는 일**을 같은 완료 상태로 합치지 않기 위해 비교한다.
...
| [[summaries/2026-04-17-cart-total-array-some|2026-04-17]]~[[summaries/2026-04-20-order-list-scenario|04-20]] | client `user.role`과 query role로 주문 범위·버튼을 나눴지만 endpoint별 role/소유자 검사는 확인되지 않았다. | `OrderList`, role·memberId parameter, 완료·취소 UI/API |
| [[summaries/2026-05-19-aam-ape-authentication-filingbox|2026-05-19]] | 후속 Passwordless 과목에서 인증·인가 용어와 기업형 인증 관리 맥락을 다시 구분했다. | AAM·APE 수업의 인증 개념 |
...
인증과 인가는 대체재가 아니라 순서상 함께 쓰는 책임이다. 먼저 credential 또는 token을 검증해 `Authentication`을 만들고, 그 다음 서버가 그 principal·authority와 요청 자원의 소유 관계를 이용해 인가해야 한다. Passwordless도 인증 방식을 앱 승인·외부 인증 서버 쪽으로 바꾸지만, 성공한 사용자가 어떤 Spring endpoint를 실행할 수 있는지는 여전히 별도 인가 정책이 필요하다.
...
- **Passwordless 후속:** 05-19는 인증·인가 용어의 후속 학습 경계다. X1280의 QR·앱 승인·외부 인증 서버 상세는 [[comparisons/passwordless-vs-password-login|Passwordless 로그인 vs 비밀번호 로그인]]에서 비교하며 FrontEnd_BackEnd 구현으로 소급하지 않는다.
