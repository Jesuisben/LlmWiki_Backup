...
2026-04-29 수업에서 local image 준비 → login → tag → push → 다른 환경의 pull/run 명령이 독립된 7교시 실습으로 이어졌고, 잘못된 local 이름으로 실행했을 때의 접근 오류도 함께 관찰했다. 독립 검색 역할과 troubleshooting 가치가 충분하므로 기존 Entity의 한 문단에만 흡수하지 않고 Concept로 보존한다.
...
다른 환경에서 namespace 없이 local 이름만으로 container를 실행하려 했을 때 local image를 찾지 못했고 repository 접근 오류가 발생했다. 이어 registry namespace를 포함한 pull/run 명령이 올바른 예로 기록됐다.
...
6. **다른 환경 pull 명령 기록**: 올바른 namespace를 사용했지만 성공 출력은 보존되지 않았다.
7. **다른 환경 run 명령 기록**: container 생성 명령이 있으나 process·상태 결과는 보존되지 않았다.
8. **application 동일성 확인**: browser·내부 file을 다시 확인해야 하지만 이 결과도 원본에 독립 보존되지 않았다.
