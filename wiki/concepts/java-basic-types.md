...
- 총정리의 `14 / 5`, `(double) 14 / 5`, `(double) (14 / 5)` 비교는 형변환 시점이 계산 결과를 바꾼다는 대표 예시다. 정수 나눗셈이 먼저 끝난 뒤 형변환하면 이미 버린 소수 부분은 돌아오지 않는다.

## 학습 연결과 범위

- **선행:** 변수 선언·대입과 `System.out.println()`으로 값을 확인하는 법.
- **후속:** 비교 결과 `boolean`은 조건문으로, `Product` 같은 참조 타입은 클래스·객체와 메모리 관점으로 이어진다.
- **Java 수업 직접 범위:** 타입 선언, 기본값, 형변환, 정수/실수 계산. DB 컬럼 타입과의 비교는 이후 Oracle·Spring에서 확장되는 관점이다.
