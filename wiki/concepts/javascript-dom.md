...
이번 백필에서는 `ProductList.html`과 `ProductDetail.html`을 실제 확인했다. `ProductList.html`은 `products` 배열을 `forEach`로 반복해 `createElement("div")`, `className`, template string, `appendChild`로 Bootstrap 카드 목록을 만든다. ADMIN 권한이면 `admin-${item.id}` 영역에 수정/삭제 버튼을 추가한다.

`ProductDetail.html`은 `loadingView`, `errorView`, `successView`를 만들고 `style.display`를 `block`/`none`으로 바꿔 상태별 화면을 전환한다. 이는 이후 React의 loading/error/success 상태 분기와 연결되는 중요한 전 단계다. URL parameter와 GET/POST 구분은 [[comparisons/get-vs-post|GET vs POST]]로 분리했다.
