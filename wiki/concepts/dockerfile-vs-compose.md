...
| 결과 | Docker image | 여러 service/container/network/volume 실행 구성 |
| 주 사용 명령 | `docker build` | `docker compose up`, `docker compose down` |
| 수업 예시 | `pohang-img`, Spring Boot 이미지 빌드 | MySQL + Spring Boot 구성 실행 |
...
- Compose가 Dockerfile을 대신하는 것은 아니다. Compose 안에서 Dockerfile로 이미지를 build하도록 지시할 수도 있다.
- Dockerfile은 단일 이미지의 레시피, Compose는 여러 컨테이너 실행의 설계도에 가깝다.
- 둘 다 YAML/텍스트 파일처럼 보이지만, 읽는 명령과 목적이 다르다.
